...
from multidim_gpflow import MdGpflowGP
...
MgGP_global_gp = MdGpflowGP
MgGP_expert_gp = MdGpflowGP
MgGP_trans_gp = MdGpflowGP
...
logfile = "/home/shahbaz/Research/Software/model_learning/Results/blocks_exp_preprocessed_data_rs_1_gpflow.p"
...
if fit_moe:
    ...
    if not load_transition_gp:
        ...
        for trans_data in trans_dicts:
            ...
            trans_dicts[trans_data]['mdgp'] = mdgp
            del mdgp
    ...
    if not load_svms:
        ...
        for label in labels:
            ...
            xu = np.array(xu)
            i = list(i)
            cnts_list = list(Counter(i).items())
            svm_check_ok = True
