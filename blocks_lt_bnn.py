...
result_file = "/home/shahbaz/Research/Software/model_learning/Results/results_blocks_bnn.p"
...
for h in horizons:
    ...
    for r in range(n_repeat):
        ...
        print('bnn 1step test score:', bnn_test_score)

        # traj_samples = model.sample_traj(x_mu_t, x_var_t, H=T, pol=None, U=U0_t_train, n_particles=100)
        traj_samples = model.sample_traj(x_mu_t, x_var_t, H=h, pol=pol, U=None, n_particles=num_traj_samples, delta_model=delta_model)
        traj_with_BNN.sample_trajs = traj_samples
        nll_mean, nll_std, rmse, X_test_log_ll = traj_with_BNN.estimate_gmm_traj_density(dpgmm_params, Xs_t_test, plot=True)
